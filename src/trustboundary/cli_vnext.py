...
def mtls_identity(
    ...
    try:
        payload = TrustIntelligence(
            TrustBoundaryEngine(wp(workspace, root))
        ).mtls_identity(
            node_id=node_id,
            spiffe_id=spiffe_id,
            san=san,
...
def assertion_library(
    ...
    payload: dict[str, object] = {
        "installed": intelligence.install_assertion_library(node_id)
    }
    if evaluate:
...
def architecture_import(
    path: Path,
    provider: ArchitectureProvider = typer.Option(
        ...,
        "--provider",
