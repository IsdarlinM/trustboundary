...
def observation(
    ...
    *,
    ...
    return WebSocketTrustObservation(
        ...
        validator=validator,
        accepted=accepted,
        evidence_ids=[] if stage is WebSocketTrustStage.HANDSHAKE else [f"E-{observation_id}"],
    )
...
def test_missing_authentication_remains_unknown() -> None:
    values = [item for item in complete_path() if item.stage is not WebSocketTrustStage.AUTHENTICATION]
...
def test_message_after_revocation_is_hypothesis_only() -> None:
...
def test_authentication_requires_validator_and_evidence() -> None:
